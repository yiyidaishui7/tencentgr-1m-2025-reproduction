...
> 在 100 万用户、478 万 Item 索引和 66 万候选的公开数据上，我把比赛 Baseline 扩展成可审计的生成式推荐实验系统；用同协议对照验证 HSTU 与容量扩展的收益，并把一次 SID 负结果定位、修复和重新验收到统计持平。
...
| 技术面试开场 | [3 页面试展示稿](INTERVIEW_DECK_CN.md) 与 [逐页讲稿](INTERVIEW_SCRIPTS_CN.md) | 3 分钟 |
...
3. 第一轮 SID 因 225 万 Item pair 碰撞和等权辅助损失导致综合分下降 5.87%。零碰撞映射、首个 Epoch 线性 warm-up 与 0.02 权重把结果修复到 0.08346，相对无 SID 的点估计为 +0.14%，区间跨 0，因此结论是“修复到持平”。
...
| -5.87% → 持平 | 是否形成失败诊断闭环 | 0.02 相对无 SID 仅 +0.14% 点估计，区间跨 0 |
