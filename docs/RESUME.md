...
- 从训练后 Item Tower 生成两级语义 ID，将 478 万 Item 的 pair 碰撞由 **2,250,041 降至 0**；通过线性 warm-up 与 0.02/0.05 权重消融，把第一轮 SID 的 **-5.87%** 退化修复到与无 SID 统计持平，并实现断点恢复、逐行预测对齐、SHA-256 回读与 GitHub/Hugging Face 分层发布。
...
- 构建模型派生两级 SID 并诊断首轮负结果：将 225 万碰撞降至 0，以首个 Epoch 线性 warm-up 和 0.02/0.05 辅助权重把 -5.87% 退化修复到持平；补齐候选过滤与历史过滤的同 checkpoint 2×2 重评，明确 +2.61% 属于评测协议效应。同步搭建原子断点、哈希回读、远端清理和公共代码/私有大产物治理流程。
...
- 从 Item Tower 表征构造两级 SID，使用容量平衡重分配把 478 万 Item 的 pair 碰撞降至 0；以权重与 warm-up 消融解释全候选排序和 Beam 生成召回的目标冲突。
...
- Generated frozen two-level semantic IDs from trained item-tower embeddings and reduced pair collisions from **2.25M to zero** across 4.78M items. Linear warm-up and 0.02/0.05 loss-weight ablations recovered an initial **-5.87%** regression to a statistical tie with the no-SID control.
...
最值得讲的是 SID 负结果。第一轮辅助目标让综合分下降 5.87%，我定位到 225 万 pair 碰撞和辅助损失过强，随后实现零碰撞映射、首个 Epoch warm-up 与两档权重，把结果修复到与无 SID 持平。整个过程使用逐行配对、哈希回读和分层发布，所以每个结论都能回到配置、预测和机器可读指标。
...
- SID 0.02 相对无 SID 的历史协议点估计为 +0.14%，配对区间跨 0；对齐协议下的 SID 差异也跨 0，所以简历写“修复到持平”。
