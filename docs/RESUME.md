...
- 重构硬编码设备与路径依赖，支持 CPU/CUDA/Ascend，增加确定性数据划分、断点
  恢复、PyTorch 精确内积召回、候选 ID 对齐审计及无多模态特征消融，形成端到端
  可复现流水线。
...
- Reworked device/path assumptions for CPU, CUDA, and Ascend; added seeded data
  ...
  audits, and a no-multimodal-feature ablation.
...
baseline and an ablation, verified all downloaded artifacts with SHA-256, and
published a tensor-equivalent SafeTensors checkpoint. The key lesson was that
evaluation contracts and ID alignment can matter as much as model code in a
large recommendation pipeline.

## Claims to avoid

- Do not describe this as a competition placement or official leaderboard score.
- Do not claim the ablation proves multimodal features are ineffective; it was
  not run through the same formal Top-10 evaluation.
