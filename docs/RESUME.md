...
- 基于 2025 腾讯广告算法大赛官方 Baseline 与 TencentGR-1M，完成 SASRec
  风格全模态序列推荐的训练、66 万候选编码、精确 Top-10 召回及最后点击留出
  评估；覆盖 78,921 名有效用户，最佳点估计 HR@10 0.03370、NDCG@10 0.01721。
- 重构硬编码设备与路径依赖，支持 CPU/CUDA/Ascend，增加确定性数据划分、断点
  恢复、精确内积召回与候选 ID 审计；设计 maxlen 101/50 × MM/no-MM 受控实验，
  no-MM50 综合分 0.02232，较 MM101 点估计提升 7.65%，并用逐行配对区间、
  交互项和历史切片约束单随机种子结论。
...
- Reproduced the 2025 Tencent Ads SASRec-style multimodal recommendation
  baseline end to end on TencentGR-1M, encoding 660k candidates and evaluating
  78,921 eligible users; the best fixed-seed point estimate reached HR@10
  0.03370 and NDCG@10 0.01721 under a leakage-aware last-click protocol.
- Reworked device/path assumptions for CPU, CUDA, and Ascend; added seeded data
  ...
  audits, and a row-aligned maxlen 101/50 × MM/no-MM study; no-MM50 scored
  0.02232 (+7.65% vs. MM101), with paired intervals, interaction, and history
  slices used to constrain the single-seed interpretation.
...
population counts and history-length slices. Finally, I ran a row-aligned
maxlen 101/50 by MM/no-MM study. no-MM50 was the best point estimate at
0.02232, 7.65% above MM101. The gain was concentrated in the 81+ history
slice, while the overall interaction interval crossed zero and every model had
one training seed, so I treated it as a recency/fusion hypothesis rather than
proof against multimodal information. I also
...
- Do not claim that the ablation proves multimodal features are ineffective or
  harmful. Some fixed-population paired intervals are positive, but every
  configuration has one training seed and the overall 2x2 interaction interval
  includes zero.
