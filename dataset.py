...
from candidate_utils import canonical_id_key
from runtime_utils import active_mm_emb_ids
...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        self.mm_emb_ids = active_mm_emb_ids(
            args.mm_emb_id,
            getattr(args, 'disable_mm_emb', False),
        )
    ...
    def fill_missing_feat(self, feat, item_id):
        ...
        for feat_id in self.feature_types['item_emb']:
            raw_item_id = self.indexer_i_rev.get(item_id)
            mm_key = canonical_id_key(raw_item_id)
            if item_id != 0 and mm_key in self.mm_emb_dict[feat_id]:
                mm_value = self.mm_emb_dict[feat_id][mm_key]
                if isinstance(mm_value, np.ndarray):
                    filled_feat[feat_id] = mm_value
...
def load_seq_as_list(seq_dir, batch_size=100000):
    ...
    with tqdm(total=total_rows, desc="Loading Seqs", unit=" rows") as pbar:
        ...
        for batch in scanner.to_batches():
            ...
            batch_events['item_id'] = pc.fill_null(flat_structs.field('item_id'), 0).to_numpy(
                zero_copy_only=False
            )
            batch_events['action_type'] = pc.fill_null(
                flat_structs.field('action_type'), 0
            ).to_numpy(zero_copy_only=False)
            batch_events['timestamp'] = pc.fill_null(flat_structs.field('timestamp'), 0).to_numpy(
                zero_copy_only=False
            )
