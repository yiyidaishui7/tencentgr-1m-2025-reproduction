...
以上是已验真的历史协议分数：66 万候选中包含 148,971 个冷候选，且未过滤用户历史。
新版 runner 会过滤到 511,029 个暖候选、屏蔽已看 Item，并在 Beam 不足 10 个合法候选
时使用 ANN 补位；重评结果单独报告，不追溯改写本表。归档 SID 是全局 residual 的
零碰撞近似，不是 OnePiece README 所述的严格 L1 内 L2 实现。
