...
[验收结果](docs/ONEPIECE_RESULTS.md)、
[无私有路径运行手册](docs/ONEPIECE_RUNBOOK.md) 和
[专项面试问答](docs/ONEPIECE_INTERVIEW_CN.md)。两条正式运行均已通过清单哈希、
78,921 行冻结协议对齐和指标重算。

| 架构 | 参数量 | HR@10 | NDCG@10 | 综合分 | 训练耗时 |
|---|---:|---:|---:|---:|---:|
...
在固定 seed 的严格单变量对照中，HSTU 综合分相对提高 4.10%。该逐用户区间只覆盖
固定评测人口，不包含训练 seed 不确定性。分桶显示 HSTU 在历史长度不超过 80 时领先，
但在 81+ 桶低 0.93%，完整结论边界见专项报告。
